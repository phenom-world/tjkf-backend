    ...
    if(req.user._id.toString() === req.body.toId ){
        try{
            ...
            if(request.length > 0){
                ...
                if (!(isFriend.length > 0)) {
                    await currentUser.updateOne({$push: { friends: { _id: request[0].fromId, username: otherUser.username, profilePhoto: otherUser.profilePhoto} }});
                    await otherUser.updateOne({$push: { friends: { _id: request[0].toId, username: currentUser.username, profilePhoto: currentUser.profilePhoto} }})
