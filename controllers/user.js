          ...
          User.findById(id)
            .then((user) => {
              ...
              mailSender(
                ...
                user,
                res,
                "Team JKF: Email Verification Successful",
                "Team JKF: Email Verification Successful",
                "Your account is now verified at Team JKF.",
                token
              );
            })
            .catch((err) => {
            ...
            (response, err) => {
              User.findById(data._id)
                .then((user) => {
                  return res
                    .status(200)
                    .json({ success: true, data: generateToken(user._id) });
                })
                .catch((err) => {
                  res.status(401);
                  throw new Error(`${err}`);
        ...
        (response, err) => {
          User.findById(user._id).then((user) => {
            ...
            mailSender(
              ...
              user,
              res,
              "Team JKF: Reset Password",
              "Team JKF: Reset Password",
              "You are receiving this mail because you or someone else has requested the reset of password"
            );
          });
        }
      );
...
exports.getAllUsers = asyncHandler(async (req, res) => {
  try {
    ...
    const currentUser = await User.findById(req.user._id);
    const usersRequiredDetails = await Promise.all(
      users.map(async (user) => {
        ...
        const request = await Relationships.find({
          partiesInvolved: {
            $all: [user._id.toString(), req.user._id.toString()],
          },
        });
        let val = isFriend.length > 0 ? true : false;
        const isRequest =
    ...
    const filteredUsers = usersRequiredDetails.filter(
      (me) => me.id.toString() !== req.user._id.toString()
    );
